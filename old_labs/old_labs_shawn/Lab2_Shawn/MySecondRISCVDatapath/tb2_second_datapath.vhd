...
-- Shawn Robinson 
-------------------------------------------------------------------------
-- tb2_second_datapath.vhd
-------------------------------------------------------------------------
-- DESCRIPTION: A second attempt at the testbench to get a different answer at the end
--
-- NOTES: 
-------------------------------------------------------------------------
...
entity tb2_second_datapath is
  generic(gCLK_HPER : time := 50 ns);
end tb2_second_datapath;

architecture behavioral of tb2_second_datapath is
...
begin
  ...
  begin
    ...
    -- #1 addi x25, x25, 0  (x25 = x25 + 0, establishes base of A)
    ...
    -- #2 addi x26, x26, 256  (x26 = x26 + 256, base of B)
    wait until falling_edge(s_clk_dp);
    ...
    s_add_sub_select  <= '0';      -- add
    s_write_enable    <= '1';
    s_MemWrite        <= '0';      -- not writing to memory 
    s_MemToReg        <= '0';      -- ALU result -> register
  ...
  --  s_rs1_address     <= "11011";  -- x27
   -- s_immediate_value <= x"200";   -- 512
    ...
    -- #4 lw x2, 4(x25)  -> x2 = A[1]  (word offset 1)
    wait until falling_edge(s_clk_dp);
    s_rd_address      <= "00010";  -- x2
    s_rs1_address     <= "11001";  -- x25
    s_immediate_value <= x"004";   -- word offset 1
    s_ALUsrc          <= '1';      -- add immediate to base
    s_add_sub_select  <= '0';      -- add
    s_write_enable    <= '1';
    s_MemWrite        <= '0';
    ...
    s_add_sub_select  <= '0';	   -- adding 
    s_write_enable    <= '0';      -- no register write
    s_MemWrite        <= '1';      -- write to memory
    s_MemToReg        <= '0';

    -------------------------------------------------------
    -- #7 lw x2, 8(x25)  -> x2 = A[2]  (word offset 2)
    -------------------------------------------------------
    wait until falling_edge(s_clk_dp);
    s_rd_address      <= "00010";  -- x2
    s_rs1_address     <= "11001";  -- x25
    s_immediate_value <= x"008";   -- word offset 2
    s_ALUsrc          <= '1';
    s_write_enable    <= '1';
    s_MemWrite        <= '0';
    s_MemToReg        <= '1';
    ...
    -------------------------------------------------------
    -- #9 sw x1, 4(x26)  -> B[1] = x1  (word offset 1)
    -------------------------------------------------------
    wait until falling_edge(s_clk_dp);
    s_rs1_address     <= "11010";  -- x26
    s_rs2_address     <= "00001";  -- x1
    ...
    -------------------------------------------------------
    -- #10 lw x2, 12(x25)  -> x2 = A[3]  (word offset 3)
    -------------------------------------------------------
    wait until falling_edge(s_clk_dp);
    s_rd_address      <= "00010";
    s_rs1_address     <= "11001";
    s_immediate_value <= x"00C";   -- word offset 3
    s_ALUsrc          <= '1';
    s_write_enable    <= '1';
    s_MemWrite        <= '0';
    s_MemToReg        <= '1';
    ...
    -------------------------------------------------------
    -- #12 sw x1, 8(x26)  -> B[2] = x1  (word offset 2)
    -------------------------------------------------------
    wait until falling_edge(s_clk_dp);
    s_rs1_address     <= "11010";
    s_rs2_address     <= "00001";
    ...
    -------------------------------------------------------
    -- #13 lw x2, 16(x25)  -> x2 = A[4]  (word offset 4)
    -------------------------------------------------------
    wait until falling_edge(s_clk_dp);
    s_rd_address      <= "00010";
    s_rs1_address     <= "11001";
    s_immediate_value <= x"010";   -- word offset 4
    s_ALUsrc          <= '1';
    s_write_enable    <= '1';
    s_MemWrite        <= '0';
    s_MemToReg        <= '1';
    ...
    -------------------------------------------------------
    -- #15 sw x1, 12(x26)  -> B[3] = x1  (word offset 3)
    -------------------------------------------------------
    wait until falling_edge(s_clk_dp);
    s_rs1_address     <= "11010";
    s_rs2_address     <= "00001";
    ...
    -------------------------------------------------------
    -- #16 lw x2, 20(x25)  -> x2 = A[5]  (word offset 5)
    -------------------------------------------------------
    wait until falling_edge(s_clk_dp);
    s_rd_address      <= "00010";
    s_rs1_address     <= "11001";
    s_immediate_value <= x"014";   -- word offset 5
    s_ALUsrc          <= '1';
    s_write_enable    <= '1';
    s_MemWrite        <= '0';
    s_MemToReg        <= '1';
    ...
    s_rd_address      <= "00010";
    s_rs1_address     <= "11001";
    s_immediate_value <= x"018";   -- word offset 6
    s_ALUsrc          <= '1';
    s_write_enable    <= '1';
    s_MemWrite        <= '0';
    s_MemToReg        <= '1';
    ...
    s_immediate_value <= x"FFC";   -- word offset -1
    s_ALUsrc          <= '1';
    s_add_sub_select  <= '0';
    s_write_enable    <= '0';
    s_MemWrite        <= '1';
   ...
   --lw x1, 16(x2)  --- NEW INSTRUCTION FROM DEMO
    wait until falling_edge(s_clk_dp);
    s_rd_address      <= "00001"; -- x1
    s_rs1_address     <= "00010";  -- x2
    s_rs2_address     <= "00000";  -- x1 (data to store)
    s_immediate_value <= x"010";   -- word offset -1
    s_ALUsrc          <= '1';
    s_add_sub_select  <= '0';
    s_write_enable    <= '1';
    s_MemWrite        <= '0';
