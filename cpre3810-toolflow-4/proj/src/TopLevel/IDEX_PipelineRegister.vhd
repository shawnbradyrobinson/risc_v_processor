...
port(	iCLK: 		in std_logic;
	...
	ID_pred_taken:		in std_logic;         -- from IFID register output
	...
	EX_pred_taken:		out std_logic         -- carries prediction to EX stage
); 
...
begin 
  ...
  s_WE_gated <= iWE or iFLUSH; 
    ...
    -- On flush: force to '0' so the NOP bubble entering EX stage never
    -- looks "predicted taken", which would cause a spurious mispredict.
    REG_pred_taken: register_NBit
        generic map(N => 1)
        port map(	D   => (0 => (ID_pred_taken and (not iFLUSH))),
			RST => iRST,
			WE  => s_WE_gated,
			CLK => iCLK,
			Q(0)=> EX_pred_taken);
