...
port(	iCLK: 		in std_logic;
	...
	IF_pred_taken:	in std_logic;          -- branch predictor output

	--ID OUTPUTS
	ID_PC:		out std_logic_vector(31 downto 0); 
	ID_Instr: 	out std_logic_vector(31 downto 0);
	ID_PC_Plus4:	out std_logic_vector(31 downto 0);
	ID_pred_taken:	out std_logic          -- carries prediction to ID stage
); 
...
architecture structural of IFID_PipelineRegister is 
	...
	-- Branch predictor: flush to '0' so NOP bubbles never look "predicted taken"
	signal s_pred_taken_in	: std_logic;
...
begin 
  ...
  --s_Instr_in <= x"00000013"          when iFLUSH = '1' else IF_Instr;
	...
	-- ── Branch predictor: pred_taken pass-through ─────────────────
	-- On flush: force to '0' so the bubble never triggers a spurious
	-- mispredict when it eventually reaches the EX stage.
	s_pred_taken_in <= '0' when iFLUSH = '1' else IF_pred_taken;

	REG_pred_taken: register_NBit
		generic map(N => 1)
		port map(
			D   => (0 => s_pred_taken_in),
			RST => iRST,
