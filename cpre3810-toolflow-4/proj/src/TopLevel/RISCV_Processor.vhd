...
F_UNIT_SW: sw_fetch_unit
  ...
  port map(iCLK               => iCLK,
           iRST               => iRST,
           i_mispredict_target => s_mispredict_target,
           i_mispredict        => s_mispredict,
           i_stall             => s_stall,
           i_update_en         => s_pred_update_en,
           i_update_PC         => s_EX_PC,
           i_actual_taken      => s_actual_taken,
           i_actual_target     => s_final_target,
           o_pred_taken        => s_IF_pred_taken,
           o_PC                => s_PC,
           Pc_plus4            => s_PC_plus4);
...
IFID_REG: IFID_PipelineRegister
	port map(
		...
		IF_pred_taken	=> s_IF_pred_taken,   -- from fetch unit predictor
		
		ID_PC		=> s_ID_PC,
		ID_Instr	=> s_ID_Instr,
		ID_PC_Plus4	=> s_ID_PC_Plus4,
		ID_pred_taken	=> s_ID_pred_taken);  -- to IDEX register
...
IDEX_REG: IDEX_PipelineRegister
	port map(
		...
		ID_pred_taken	=> s_ID_pred_taken,	-- from IFID register output
		...
		EX_Halt		=> s_EX_Halt,
		EX_pred_taken	=> s_EX_pred_taken	-- to mispredict checker
	);
