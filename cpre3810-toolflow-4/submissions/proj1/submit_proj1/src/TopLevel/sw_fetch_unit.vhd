...
architecture structure of sw_fetch_unit is
	...
	signal s_stall_inverted		: std_logic; -- avoiding behavioral stuff with an inverted signal
	
	--signal s_WE_PC			: std_logic; 
...
--  s_imm_shifted <= immediate_generate; 
  ...
  o_PC 		<= s_current_PC; 
  PC_plus4	<= s_pc_plus4; 
 -- s_WE_PC	<= '1' when iRST = '1' else s_stall_inverted; 
 -- s_pc_reg_input <= c_PC_RESET when iRST = '1' else s_next_PC;
  s_PC_WE	<= s_stall_inverted or i_taken;
