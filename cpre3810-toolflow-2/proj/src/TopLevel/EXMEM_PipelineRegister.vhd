...
architecture structural of EXMEM_PipelineRegister is 
	...
	-- 2: MemRead
	-- 1: MemWrite
	signal s_ctrl_in	: std_logic_vector(4 downto 0);
	signal s_ctrl_out	: std_logic_vector(4 downto 0);

begin 


  -- Pack control inputs
  s_ctrl_in <= EX_RegWRite &
               EX_MemToReg &
               EX_MemRead  &
               EX_MemWrite;

  -- Unpack control outputs
  MEM_RegWrite  <= s_ctrl_out(4);
  MEM_MemToReg  <= s_ctrl_out(3 downto 2);
  MEM_MemRead   <= s_ctrl_out(1);
  MEM_MemWrite  <= s_ctrl_out(0);
