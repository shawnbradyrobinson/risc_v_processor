...
signal s_EX_ALUSrc	: std_logic_vector(31 downto 0);
...
s_EX_funct3	<= EX_Instr(14 downto 12);
s_EX_funt7_5	<= EX_Instr(30);
s_EX_rd_addr	<= EX_Instr(11 downto 7);
s_EX_rs1_addr	<= EX_Instr(19 downto 15); -- FOR FORWARDING!
s_EX_rs2_addr	<= EX_Instr(24 downto 20); -- FOR FORWARDING!


--MEM OUTPUT SLICING
s_MEM_funct3	<= MEM_Instr(14 downto 12);
...
with s_MemToReg select
  ...
  DMem: mem
    generic map(ADDR_WIDTH => ADDR_WIDTH,
                DATA_WIDTH => N)
    port map(clk  => iCLK,
             addr => s_MEM_ALU_Result(11 downto 2), -- pipelining, bayBEE!
             data => s_MEM_rs2_out,
             we   => s_MEM_MemWrite,
             q    => s_DMemOut);
...
IDEX_REG: IDEX_PipelineRegister
	port map(
		...
		ID_PC		=> ID_PC 	-- from IF/ID register output
		ID_Instr	=> ID_Instr	-- from IF/ID register output
		...
		ID_isAUIPC	=> s_isAUIPIC,	-- from concurrent assign
...
EXMEM_REG: EXMEM_PipelineRegister
	port map(
		...
		--datapath inputs--
		EX_PC		=> EX_PC,	--from ID/EX output
		EX_Instr	=> EX_Instr,	--from ID/EX output
		EX_ALU_Result	=> s_ALUOut,	--from ALU output
		EX_rs2_out	=> EX_rs2_out,	--from ID/EX output
		EX_rd_addr	=> s_EX_rd_addr,--from ID/EX slice
		
		--Control inputs-- 
		EX_MemWrite	=> EX_MemWrite,	--from ID/EX output
		EX_MemRead	=> EX_MemRead,	--from ID/EX output
		EX_RegWrite	=> EX_RegWrite,	--from ID/EX output
		EX_MemToReg	=> EX_MemToReg,	--from ID/EX output
